...
def multi2single(gdf):
    ...
    for i, row in gdf_multipoly.iterrows():
        ...
        df = pandas.concat([geopandas.GeoDataFrame(row, crs=gdf_multipoly.crs).T]*len(Series_geometries), ignore_index=True)
...
@click.option("--in_id", "-id", help="Unique id of in_file or in_layer", default="station")
def merge(wksp, in_id):
    ...
    dissolved["geometry"] = (
        dissolved.buffer(.1).buffer(-.1)
    )
    out = dissolved.reset_index()
    out.to_file('watersheds.gpkg', layer='watersheds_src', driver="GPKG")

    # rather than monkey with shapely geoms, remove holes by extracting exterior ring using ogr
    cmd = [
	    "ogr2ogr",
	    "-f",
	    "GPKG",
	    ...
	    "watersheds",
	    "-sql",
	    "SELECT station, wscode, localcode, refine_met, ST_MakePolygon(ST_ExteriorRing(geom)) as geom FROM watersheds_src",
	    "-dialect",
	    "SQLITE",
	    "watersheds.gpkg"
    ]
